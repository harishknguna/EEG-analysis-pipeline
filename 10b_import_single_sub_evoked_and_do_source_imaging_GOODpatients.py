...
"""
=============================================
10b. For patients: Single sub source reconstruction using eeg template MRI
...
###########################################
DBSinfo = ['ON']  # run the code separately for ON/OFF
###########################################
...
for subject in config_for_gogait.subjects_list_patients:     
    ...
    for ci, condi in enumerate(condi_name_all):
        ...
        extension = condi_name_all[ci] +'_' + event_cue +'_' + dbs +'_' + version + '_epo'
...
#%%
    ...
    #%%            
            ...
            #%% STEP 2: Compute regularized noise covariance per subject (same for all condi)
                ...
                extension =  dbs +'_' + version +'_cov'
                fname_out = op.join(eeg_subject_dir_GOODpatients,
                                     config_for_gogait.base_fname.format(**locals()))
               ...
               ##  if fixed == 'False', and loose == 0.2, same as case 1  if orient = "vector"
                ...
                extension = dbs + '_' + version + '_' + orientation + '_inv' 
                fname_out = op.join(eeg_subject_dir_GOODpatients,
                                     config_for_gogait.base_fname.format(**locals()))
           ...
           #%% STEP 4: Compute inverse solution per condi and per sub
            ...
            evoked = epochs.average().pick("eeg").set_eeg_reference(projection=True)
    ...
    report.title = 'Forward_model_and_noise_cov: ' + subject + '_at_'+ evnt +'_' + dbs +'_' + version +'_' + orientation + '_' + method 
    report_fname = op.join(config_for_gogait.report_dir_GOODpatients, subject)
    report.save(report_fname+'_fwd_std1005_noisecov_cue_' + dbs +'_' + orientation + '_' + version + '_' + method + '.html', overwrite=True)
