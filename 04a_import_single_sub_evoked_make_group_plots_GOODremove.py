...
"""
=============================================
04. import single sub evoked and make GROUP Evoked plots, GFP, and topomaps 
==============================================
...
all condition in one html file. 
...
for ei, evnt in enumerate(event_type):
    ...
    for ci, condi in enumerate(condi_name): 
        ...
        evoked_fname = op.join(config_for_gogait.eeg_dir_GOODremove,
    ...
    report.title = 'Group sub (n = '+ str(n_subs)+ ') evoked : ' + evnt+ '_' + version + '_' + ampliNormalization
    #report.title = 'Group sub STC contrast at ' + evnt
    extension = 'group_sub_evoked'
    report_fname = op.join(config_for_gogait.report_dir, config_for_gogait.base_fname_generic.format(**locals()))
    report.save(report_fname+'_' + evnt +'_' + ampliNormalization + '_' + version+ '.html', overwrite=True)            
