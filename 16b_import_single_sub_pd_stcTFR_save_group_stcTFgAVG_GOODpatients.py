...
"""
=============================================
16b. For patients: Imports single sub stcTFR files and make group stcTFR (average and SD across subjects)
==============================================
...
for ei, evnt in enumerate(event_type):
    ...
    for di, dbs in enumerate(DBSinfo): 
        ...
        # for ci, condi in enumerate(condi_name):
        #     print('\n Writing the stcTFgAVG to disk: %s %s'  % (condi, dbs))
        ...
        #     print('\n Writing the stcTFgSTD to disk: %s %s'  % (condi, dbs))
        #     extension = condi_name[ci] +'_' + event_type[ei] + '_pd_' + dbs +'_stcTFgSTD_bslnCorr' + '_' + freq_band_name+ '_' + version +'_' + method +'_' + ampliNormalization 
        #     stc_fname_array = op.join( config_for_gogait.eeg_dir_GOODpatients,
        #                           config_for_gogait.base_fname_avg_npy.format(**locals()))
        #     print("Output: ", stc_fname_array)
