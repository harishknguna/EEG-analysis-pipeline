...
"""
=============================================
13c. For patients: Imports single sub STC files, do single sub DBS contrast (ON > OFF), and make group (average and SD across subjects)
==============================================
Imports single sub STC files, do the norm, then do the baseline correction, do the DBS contrast (ON > OFF)
...
## note: here we do dbs contrast: ON > OFF
...
for ei, evnt in enumerate(event_type):
    ...
    for veri, version in enumerate(version_list):
        for di, dbs in enumerate(DBSinfo):
            ...
            # store DBS in dim 1, sub in dim2, condi in dim3, vertices in dim4, time in dim5 
...
for ci, condi in enumerate(condi_name): 
    ...
    stc_contra_data = stc_data_all_sub_all_condi_dbs[1,:,ci,:,:] - stc_data_all_sub_all_condi_dbs[0,:,ci,:,:] 
    ...
    print('\n Writing the stcDBScontraGAVG to disk: %s '  % condi)
    ...
    print('\n Writing the stcDBScontraGSTD  to disk: %s'  % condi)
    extension = condi_name[ci] +'_ON_OFF_' + event_type[ei] +'_pd_'+'stcGSTD'+'_evkDBSContrast_SRC' + '_' + version +'_' + method +'_' + ampliNormalization +'_' + bsln 
    stc_fname_array = op.join( config_for_gogait.eeg_dir_GOODpatients,
                          config_for_gogait.base_fname_avg_npy.format(**locals()))
    print("Output: ", stc_fname_array)
