...
"""
=============================================
18a. Imports single sub stcTFR files, do single sub contrast, and make group (average and SD across subjects)
==============================================
Imports single sub stcTFR files, do the norm, then do the baseline correction, do the contrast  
and finally make group STC by conditions across subjects generated in the previous code 15a 
...
################# CHANGE HERE ########################################
freq_band_name ='theta' #'theta' |'beta_low'
######################################################################
...
#%% taking contrast of TF at source space and saving them
    ...
    # for cti, contra in enumerate(contrast_kind):
    #     if contra == 'GOu_GOc':
    #         stc_contra_data =  stc_data_all_sub_all_condi[1,:,:,:] -  stc_data_all_sub_all_condi[0,:,:,:]
    #     elif contra == 'NoGo_GOu':
    #         stc_contra_data =  stc_data_all_sub_all_condi[2,:,:,:] -  stc_data_all_sub_all_condi[1,:,:,:]
    ...
    #     print('\n Writing the stcContraTFgAVG to disk: %s'  % contra)
    #     extension = contra +'_' + event_type[ei] + '_hc_' +'stcContraTFgAVG_bslnCorr' + '_' + freq_band_name+ '_' + version +'_' + method +'_' + ampliNormalization 
    ...
    #     print('\n Writing the stcContraTFgSTD to disk: %s'  % contra)
    #     extension = contra +'_' + event_type[ei] + '_hc_' +'stcContraTFgSTD_bslnCorr' + '_' + freq_band_name+ '_' + version +'_' + method +'_' + ampliNormalization 
    #     stc_fname_array = op.join( config_for_gogait.eeg_dir_GOODremove,
    #                           config_for_gogait.base_fname_avg_npy.format(**locals()))
    #     print("Output: ", stc_fname_array)
