...
"""
=============================================
11a. For patients: Imports single sub STC files and make group STC (average and SD across subjects)
==============================================
Imports single sub STC files, do the norm, then do the baseline correction, 
...
for ei, evnt in enumerate(event_type):
    ...
    for di, dbs in enumerate(DBSinfo):
        ...
        stc_data_avg_sub_all_condi = np.nanmean(stc_data_all_sub_all_condi, axis = 0)
        ...
        #     extension = condi_name[ci] +'_' + event_type[ei] + '_pd_' + dbs +'_stcGSTD' +'_evoked_' + version +'_' + method +'_' + ampliNormalization +'_' + bsln
        #     stc_fname_array = op.join( config_for_gogait.eeg_dir_GOODpatients,
        #                           config_for_gogait.base_fname_avg_npy.format(**locals()))
        #     print("Output: ", stc_fname_array)
