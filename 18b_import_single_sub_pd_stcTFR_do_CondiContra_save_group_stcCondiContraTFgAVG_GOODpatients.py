...
"""
=============================================
18b. For patients: Imports single sub stcTFR files, do single sub contrast, and make group (average and SD across subjects)
==============================================
...
for ei, evnt in enumerate(event_type):
    ...
    for di, dbs in enumerate(DBSinfo): 
        ...
        #     print('\n Writing the stcContraTFgAVG to disk: %s %s'  % (contra, dbs))
        ...
        #     print('\n Writing the stcTFgSTD to disk: %s %s'  % (contra, dbs))
        #     extension = contra +'_' + event_type[ei] + '_pd_' + dbs +'_stcCondiContraTFgSTD_bslnCorr' + '_' + freq_band_name+ '_' + version +'_' + method +'_' + ampliNormalization 
        #     stc_fname_array = op.join( config_for_gogait.eeg_dir_GOODpatients,
        #                           config_for_gogait.base_fname_avg_npy.format(**locals()))
        #     print("Output: ", stc_fname_array)
